...
<section id="section-2">
    ...
    <ul>
        <li><strong>Source repository</strong> - Where dashboard definitions live:
            <ul>
                <li><code>redhat-appstudio/o11y/dashboards/</code> - All Integration Service dashboards are here:
                    <ul>
                        <li><code>grafana-dashboard-konflux-integration-service-slo.configmap.yaml</code></li>
                        <li><code>grafana-dashboard-konflux-integration-service-sli.configmap.yaml</code></li>
                    </ul>
                </li>
            </ul>
        </li>
        <li><strong>Deployment configuration</strong> - References which commit SHA to deploy:
            <ul>
                <li><code>app-interface/data/services/stonesoup/cicd/saas-stonesoup-dashboards.yml</code></li>
            </ul>
        </li>
    </ul>
...
# This creates a merge request in GitLab</code></pre>
    ...
    <p>Suppose you added a new panel to the Integration Service SLO dashboard (<code>grafana-dashboard-konflux-integration-service-slo.configmap.yaml</code>) in the <code>redhat-appstudio/o11y</code> repo.</p>
...
# Output: abc123def456...</code></pre>
    ...
    <p>In <code>data/services/stonesoup/cicd/saas-stonesoup-dashboards.yml</code>, find the production target section:</p>
...
- name: stonesoup-dashboards
  ...
  targets:
  - namespace:
      $ref: /services/observability/namespaces/app-sre-observability-stage-int.appsres09ue1.yml
    ref: main
  - namespace:
      $ref: /services/observability/namespaces/app-sre-observability-production-int.appsrep09ue1.yml
    ref: e6da55c7a3924f6fc32e2abc2fea43eed90abcf3  # ← Update this line</code></pre>
    </div>

    <p>Change the production ref to your new SHA:</p>
    <div class="code-block">
        <button class="copy-button">Copy</button>
...
podman machine start</code></pre>
    ...
    <div class="code-block">
        <button class="copy-button">Copy</button>
        <pre><code class="language-bash">cd ~/repo/o11y  # Clone of redhat-appstudio/o11y
